...
int get_process_count(const int argc, const char** argv) {
    int res = -1;
    ...
    for(int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-p") == 0 && i + 1 < argc) {
            res = (int) strtol(argv[i+1], &endptr, 10) + 1;
        }
    }
...
int task(process_info* proc) {
    ...
    write_log(proc->log_file, log_started_fmt, proc->id, proc->pid, proc->parent);
    ...
    write_log(proc->log_file, log_received_all_started_fmt, proc->id);
...
int main(int argc, char* argv[]) {
    ...
    // Parse arguments (-p X)
    int process_count = get_process_count(argc, (const char **) argv);
